...
@interface AddShopTableViewController ()<UITextFieldDelegate>

@end

@implementation AddShopTableViewController
...
-(void)addShopToContacts {
    ...
    if (addressBook) {
        ...
        if (!person) {
            NSLog(@"Can Not Create Shop AddressBook");
            return;
        }
        ...
        if (couldSetCompanyName && coulSetNotes && couldSetPhoneNumber && emailSet) {
            ...
            for (int index=0; index<CFArrayGetCount(allGroups); index++){
                ...
                if ([groupName isEqualToString:@"To4ka"]) {
                    if (ABGroupAddMember(group, person, &error)) {
                        NSLog(@"Shop Added To Group AddressBook");
                    } else {
                        NSLog(@"Failed! Shop Failed Add To Group AddressBook");
                    }
                    break;
                }
            }
...
-(IBAction)addSecondShopPhoneNumberButtone:(id)sender {
...
-(BOOL)uniqueEntityExistsWithEnityName {
    ...
    NSSortDescriptor *shopName = [[NSSortDescriptor alloc] initWithKey:@"shopName" ascending:YES];
    
    [request setSortDescriptors: @[shopName]];
    ...
    if (!matches) {
        ...
        UIAlertView *exist = [[UIAlertView alloc]initWithTitle:@"Shop Exist in Your Records"
                                                       message:existShop
                                                      delegate:nil
                                             cancelButtonTitle:@"Ok"
                                             otherButtonTitles:nil];
        ...
        addShop.shopName = _shopName.text;
        addShop.shopPhoneNumber = _shopPhoneNumber.text;
        addShop.shopSecondPhoneNumber = _shopSecondPhoneNumber.text;
        addShop.shopEmail = _shopEmail.text;
        addShop.shopNote = _shopNoteView.text;
